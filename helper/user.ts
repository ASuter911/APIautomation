...
export function signUp(user:User):Promise<any> {
    return new Promise((resolve, reject) => {
        request.post('users/signup').send(user).end((err,res) => {
            if (err)reject(err)
            else resolve(res)
        })
    })
...
export function login(user:User):Promise<any> {
    return new Promise((resolve, reject) => {
        request.post('users/login').send(user).end((err,res) => {
            if (err)reject(err)
            else resolve(res)
        })
    })
...
export function login2(user:User){
    ...
    export function deleteFunction(cookie:string):Promise<any> {
        return new Promise((resolve, reject) => {
            request.delete('users/deleteMe').set('Cookie', cookie).end((err,res) => {
                if (err)reject(err)
                else resolve(res)
            })
        })
