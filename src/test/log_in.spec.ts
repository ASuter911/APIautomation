  ...
  describe("POSITIVE TESTING", () => {
    ...
    it("should signup, login and delete the user", async () => {
      try {
        ...
        expect(loginAfterDelete.body.message).toBe(
    ...
    it("Should signup, login and delete the user .then()", () => {
      return signUp(user)
        ...
        .then((loginAfrerDelete) => {
          ...
          expect(loginAfrerDelete.body.message).toEqual(
            "Incorrect email or password"
          );
        });
    });

    it("Should signup, login and delete the user .end", (done) => {
      signUp2(user).end((err, res) => {
        ...
        login2(user).end((err, loginRes) => {
          ...
          deleteFunction2(cookie).end((err, deleteRes) => {
            if (err) return done(err);
            expect(deleteRes.statusCode).toBe(200);
            expect(deleteRes.body.message).toEqual("User deleted successfully");
            login2(user).end((err, loginAfterDelete) => {
              if (err) return done(err);
              expect(loginAfterDelete.statusCode).toBe(401);
              expect(loginAfterDelete.body.message).toEqual(
                "Incorrect email or password"
              );
              done();
            });
